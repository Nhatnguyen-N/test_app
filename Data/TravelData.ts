import { travelItemParams } from "@/types/type"

export const travel: travelItemParams[] = [
  {
    _id: 1,
    images: [
      {
        id: 1,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new2_xijl81.jpg"
      },
      {
        id: 2,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new3_vnwkju.jpg"
      },
      {
        id: 3,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new1_m6jbnd.jpg"
      }
    ],
    title: "Travel New1",
    description: "Biển luôn là một trong những điểm đến hấp dẫn bậc nhất đối với du khách khắp nơi trên thế giới. Với bãi cát trắng mịn, làn nước trong xanh và những cơn gió mát rượi, biển không chỉ mang đến sự thư giãn mà còn giúp du khách hòa mình vào thiên nhiên trong lành. Một trong những điều tuyệt vời nhất khi du lịch biển là tận hưởng ánh bình minh hoặc hoàng hôn trên bờ cát. Hình ảnh mặt trời dần ló dạng hoặc lặn xuống chân trời, nhuộm cả bầu trời một màu cam rực rỡ, tạo nên một khung cảnh thơ mộng và yên bình. Bên cạnh đó, những hoạt động như tắm biển, lặn ngắm san hô, chèo thuyền kayak hay lướt sóng mang lại cảm giác hứng khởi và đầy năng lượng. Không chỉ có cảnh đẹp, biển còn là thiên đường ẩm thực với vô vàn món hải sản tươi ngon như tôm hùm, cua ghẹ, mực nướng hay gỏi cá trích. Mỗi vùng biển lại có những đặc sản riêng, mang đến trải nghiệm ẩm thực độc đáo cho du khách. Ngoài ra, du lịch biển còn giúp tinh thần sảng khoái, giảm căng thẳng và mang lại những phút giây thư giãn tuyệt vời. Dù là chuyến đi cùng gia đình, bạn bè hay một hành trình khám phá một mình, biển luôn là lựa chọn hoàn hảo để tái tạo năng lượng và tận hưởng cuộc sống. Hãy dành thời gian để khám phá những bãi biển đẹp, tận hưởng không khí trong lành và ghi lại những khoảnh khắc đáng nhớ! 🌊🌞"
  },
  {
    _id: 2,
    images: [
      {
        id: 4,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new2_xijl81.jpg"
      },
      {
        id: 5,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new3_vnwkju.jpg"
      },
      {
        id: 6,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new1_m6jbnd.jpg"
      }
    ],
    title: "Travel New 2",
    description: "Biển luôn là một trong những điểm đến hấp dẫn bậc nhất đối với du khách khắp nơi trên thế giới. Với bãi cát trắng mịn, làn nước trong xanh và những cơn gió mát rượi, biển không chỉ mang đến sự thư giãn mà còn giúp du khách hòa mình vào thiên nhiên trong lành. Một trong những điều tuyệt vời nhất khi du lịch biển là tận hưởng ánh bình minh hoặc hoàng hôn trên bờ cát. Hình ảnh mặt trời dần ló dạng hoặc lặn xuống chân trời, nhuộm cả bầu trời một màu cam rực rỡ, tạo nên một khung cảnh thơ mộng và yên bình. Bên cạnh đó, những hoạt động như tắm biển, lặn ngắm san hô, chèo thuyền kayak hay lướt sóng mang lại cảm giác hứng khởi và đầy năng lượng. Không chỉ có cảnh đẹp, biển còn là thiên đường ẩm thực với vô vàn món hải sản tươi ngon như tôm hùm, cua ghẹ, mực nướng hay gỏi cá trích. Mỗi vùng biển lại có những đặc sản riêng, mang đến trải nghiệm ẩm thực độc đáo cho du khách. Ngoài ra, du lịch biển còn giúp tinh thần sảng khoái, giảm căng thẳng và mang lại những phút giây thư giãn tuyệt vời. Dù là chuyến đi cùng gia đình, bạn bè hay một hành trình khám phá một mình, biển luôn là lựa chọn hoàn hảo để tái tạo năng lượng và tận hưởng cuộc sống. Hãy dành thời gian để khám phá những bãi biển đẹp, tận hưởng không khí trong lành và ghi lại những khoảnh khắc đáng nhớ! 🌊🌞"
  },
  {
    _id: 3,
    images: [
      {
        id: 7,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new2_xijl81.jpg"
      },
      {
        id: 8,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new3_vnwkju.jpg"
      },
      {
        id: 9,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new1_m6jbnd.jpg"
      }
    ],
    title: "Travel New 3",
    description: "Biển luôn là một trong những điểm đến hấp dẫn bậc nhất đối với du khách khắp nơi trên thế giới. Với bãi cát trắng mịn, làn nước trong xanh và những cơn gió mát rượi, biển không chỉ mang đến sự thư giãn mà còn giúp du khách hòa mình vào thiên nhiên trong lành. Một trong những điều tuyệt vời nhất khi du lịch biển là tận hưởng ánh bình minh hoặc hoàng hôn trên bờ cát. Hình ảnh mặt trời dần ló dạng hoặc lặn xuống chân trời, nhuộm cả bầu trời một màu cam rực rỡ, tạo nên một khung cảnh thơ mộng và yên bình. Bên cạnh đó, những hoạt động như tắm biển, lặn ngắm san hô, chèo thuyền kayak hay lướt sóng mang lại cảm giác hứng khởi và đầy năng lượng. Không chỉ có cảnh đẹp, biển còn là thiên đường ẩm thực với vô vàn món hải sản tươi ngon như tôm hùm, cua ghẹ, mực nướng hay gỏi cá trích. Mỗi vùng biển lại có những đặc sản riêng, mang đến trải nghiệm ẩm thực độc đáo cho du khách. Ngoài ra, du lịch biển còn giúp tinh thần sảng khoái, giảm căng thẳng và mang lại những phút giây thư giãn tuyệt vời. Dù là chuyến đi cùng gia đình, bạn bè hay một hành trình khám phá một mình, biển luôn là lựa chọn hoàn hảo để tái tạo năng lượng và tận hưởng cuộc sống. Hãy dành thời gian để khám phá những bãi biển đẹp, tận hưởng không khí trong lành và ghi lại những khoảnh khắc đáng nhớ! 🌊🌞"
  },

]

export const travelEN: travelItemParams[] = [
  {
    _id: 1,
    images: [
      {
        id: 10,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new2_xijl81.jpg"
      },
      {
        id: 11,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new3_vnwkju.jpg"
      },
      {
        id: 12,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new1_m6jbnd.jpg"
      }
    ],
    title: "Travel New1",
    description: "The sea has always been one of the most attractive destinations for travelers around the world. With soft white sand, crystal-clear water, and refreshing breezes, the beach not only offers relaxation but also allows visitors to immerse themselves in pristine nature. One of the most wonderful experiences when traveling to the beach is enjoying the sunrise or sunset on the shore. The sight of the sun slowly rising or setting on the horizon, painting the sky with a vibrant orange hue, creates a dreamy and peaceful landscape.Besides its breathtaking scenery, the sea offers a variety of exciting activities such as swimming, snorkeling, kayaking, and surfing, bringing a sense of thrill and energy. The beach is also a paradise for food lovers, with a wide range of fresh seafood delicacies like lobster, crab, grilled squid, and herring salad. Each coastal region has its own specialties, offering visitors unique culinary experiences.Moreover, a beach getaway helps to refresh the mind, reduce stress, and bring moments of pure relaxation. Whether it’s a trip with family, friends, or a solo adventure, the beach is always the perfect choice to recharge and enjoy life. Take the time to explore stunning beaches, soak in the fresh air, and capture unforgettable memories! 🌊🌞"
  },
  {
    _id: 2,
    images: [
      {
        id: 13,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new2_xijl81.jpg"
      },
      {
        id: 14,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new3_vnwkju.jpg"
      },
      {
        id: 15,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new1_m6jbnd.jpg"
      }
    ], title: "Travel New 2",
    description: "The sea has always been one of the most attractive destinations for travelers around the world. With soft white sand, crystal-clear water, and refreshing breezes, the beach not only offers relaxation but also allows visitors to immerse themselves in pristine nature. One of the most wonderful experiences when traveling to the beach is enjoying the sunrise or sunset on the shore. The sight of the sun slowly rising or setting on the horizon, painting the sky with a vibrant orange hue, creates a dreamy and peaceful landscape.Besides its breathtaking scenery, the sea offers a variety of exciting activities such as swimming, snorkeling, kayaking, and surfing, bringing a sense of thrill and energy. The beach is also a paradise for food lovers, with a wide range of fresh seafood delicacies like lobster, crab, grilled squid, and herring salad. Each coastal region has its own specialties, offering visitors unique culinary experiences.Moreover, a beach getaway helps to refresh the mind, reduce stress, and bring moments of pure relaxation. Whether it’s a trip with family, friends, or a solo adventure, the beach is always the perfect choice to recharge and enjoy life. Take the time to explore stunning beaches, soak in the fresh air, and capture unforgettable memories! 🌊🌞"
  },
  {
    _id: 3,
    images: [
      {
        id: 16,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new2_xijl81.jpg"
      },
      {
        id: 17,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new3_vnwkju.jpg"
      },
      {
        id: 18,
        imageUrl: "https://res.cloudinary.com/dsq3oy0yz/image/upload/v1739511494/new1_m6jbnd.jpg"
      }
    ],
    title: "Travel New 3",
    description: "The sea has always been one of the most attractive destinations for travelers around the world. With soft white sand, crystal-clear water, and refreshing breezes, the beach not only offers relaxation but also allows visitors to immerse themselves in pristine nature. One of the most wonderful experiences when traveling to the beach is enjoying the sunrise or sunset on the shore. The sight of the sun slowly rising or setting on the horizon, painting the sky with a vibrant orange hue, creates a dreamy and peaceful landscape.Besides its breathtaking scenery, the sea offers a variety of exciting activities such as swimming, snorkeling, kayaking, and surfing, bringing a sense of thrill and energy. The beach is also a paradise for food lovers, with a wide range of fresh seafood delicacies like lobster, crab, grilled squid, and herring salad. Each coastal region has its own specialties, offering visitors unique culinary experiences.Moreover, a beach getaway helps to refresh the mind, reduce stress, and bring moments of pure relaxation. Whether it’s a trip with family, friends, or a solo adventure, the beach is always the perfect choice to recharge and enjoy life. Take the time to explore stunning beaches, soak in the fresh air, and capture unforgettable memories! 🌊🌞"
  },

]